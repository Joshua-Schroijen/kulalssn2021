...
class LouvainHarness(minProgress: Int, progressCounter: Int) {
  ...
  def run[VD: ClassTag](sc: SparkContext, graph: Graph[VD, Long], graphName: String): Unit = {
		this.resultsFile = new File(s"results\\results_louvain_$graphName.txt")
    ...
    this.resultsFileWriter.println("-" * 100)
		this.resultsFileWriter.println(s"| Louvain results ($graphName)" + (" " * (79 - graphName.length)) + "|")
		this.resultsFileWriter.println("-" * 100)
  ...
  private def saveLevel(sc: SparkContext, level: Int, q: Double, graph: Graph[LouvainVertexState, Long]): Unit = {
		...
		this.resultsFileWriter.println(s"Level $level, q = $q, # communities = ${graph.vertices.count}")
		this.resultsFileWriter.println("Vertex\tCommunity\tCommunitySigmaTot\tInternalWeight\tnodeWeight")
		for (vertex <- vertices) resultsFileWriter.println(s"${vertex._1}\t${vertex._2.community}\t\t${vertex._2.communitySigmaTot}\t\t\t${vertex._2.internalWeight}\t\t${vertex._2.nodeWeight}")
		this.resultsFileWriter.println("-" * 100)
  }
}